...
def split(matrix, cut=None, split='both'):
    ...
    if (cut is not None):
        ...
        left, s, right = np.linalg.svd(matrix, full_matrices=False)
        # left, s, right = rsvd(matrix, matrix.shape[1])
        # left, s, right = svd(matrix, full_matrices=False, lapack_driver='gesvd')
        if split == 'both':
            alpha = len(s[s > 1e-14])
            left = np.dot(left, np.diag(np.sqrt(s))[:, :alpha])
...
class ThreeDimensionalTriadNetwork:
    ...
    def load_nn_imp(self, triads1, triads2):
        ...
        self.nnimp = True
        print("loaded in the nn-impure triads.")
        
    def load_imp(self, triads):
        """
        Load in the impure tensors for the local
        interaction.

        """
        self.Aimp, self.Bimp, self.Cimp, self.Dimp = triads
        self.imp = True
        print("loaded in the impure triads.")
    ...
    def getR23(self, C, D, B):
        ...
        B is structured (alpha down beta)
    ...
    def get_lognorms(self,):
        ...
        lognorms.append(np.log(np.abs(trace)))
        return lognorms
